...
对于一维奇偶校验，以偶校验为例，校验位只有 1 位，校验位的结果是使得原始一维数据加上校验位上可能的 1 之后，所有 1 的数量为偶数的情况（奇校验就是奇数）。我们将校验位记作 $P$。现在 $(M,P)$ 开始传输，经过了各种存储器和总线后我们将其表示为 $(M',P'')$。我们想知道此时的 $M'$ 和之前的 $M$ 是不是同一个东西，怎么办呢？重新拿 $M'$ 计算一次校验位得到 $P'$，比较 $P''$ 和 $P'$ 是否相同即可，如果相同，我们就认为数据传输没有发生问题，如果不相同那么数据传输就发生了问题，需要对 $M'$ 纠错后再输出。
...
而二维奇偶校验就可以纠正某一个位置上的错误了。二维奇偶校验首先将原始数据排列成一个矩形，然后给每一行和每一列都算一个校验位，如下图所示：

![将原始数据排列成一个矩形，然后给每一行和每一列都算一个校验位](https://cdn.dwj601.cn/images/20250630194304016.png)

以偶校验为例：

![偶校验结果](https://cdn.dwj601.cn/images/20250630194353556.png)

可以看出，二维奇偶校验可以分别定位单个出错比特的行号和列号，从而唯一确定出错的比特，进而进行差错纠错。

## I/O 设备

主要有：输入输出设备、外部存储设备、网络设备三类设备。
